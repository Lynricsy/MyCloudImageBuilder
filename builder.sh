...
# 临时目录路径（优先使用环境变量以兼容 CI 环境）
# 优先级：IMAGE_TEMP_DIR > RUNNER_TEMP > TMPDIR > /tmp
TEMP_DIR="${IMAGE_TEMP_DIR:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}}/.ImageMakerTemp"
...
echo "::group::Download base image"
...
# 在 GitHub Actions 中添加 --no-network 参数以避免 passt 权限问题
if [ -n "${GITHUB_ACTIONS:-}" ]; then
    log_info "🔧 检测到 GitHub Actions 环境，禁用 guest 网络以避免权限问题"
    VIRT_CUSTOMIZE_CMD="virt-customize --no-network -a debian-13-generic-amd64.qcow2"
else
    VIRT_CUSTOMIZE_CMD="virt-customize -a debian-13-generic-amd64.qcow2"
fi
...
# 如果在 GitHub Actions 中运行，则写入步骤总结
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
    {
        echo "# Build Summary"
        echo ""
        echo "- **Downloaded**: ${DOWNLOAD_SIZE}"
        echo "- **Customized**: ${CUSTOMIZE_SIZE}"
        echo "- **Final**: ${FINAL_SIZE}"
        echo ""
        echo "Output: \`debian-13-generic-amd64-NEXT.qcow2\`"
    } >> "${GITHUB_STEP_SUMMARY}"
fi